...
This project provides default local TRELLIS.2 generation, an explicitly selected Tripo API option, Blender processing and an optional Three.js viewer. Read [README.md](README.md) for scope and [INSTALL.md](INSTALL.md) for reproducible setup. For asset requests, read [.agents/skills/3d-assets/SKILL.md](.agents/skills/3d-assets/SKILL.md); for data flow, read [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md).
...
- Before Tripo submission, follow [docs/TRIPO.md](docs/TRIPO.md): plan supported inputs and estimated credits, use the user's authorized budget, and keep credentials out of specs/logs/Git. `max_credits` guards a local estimate, not server-side spending. Persist the remote task before polling; resume a known task instead of creating a replacement. An unknown submission outcome stops for reconciliation and must not trigger automatic POST retries.
...
Routine tests must not submit paid Tripo jobs. Use mock API tests for failure/recovery behavior. A live test requires usable credentials and authorized generation spend; report it separately from local tests and do not infer quality superiority from API availability.
