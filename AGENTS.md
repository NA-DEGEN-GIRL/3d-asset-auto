...
- Optional Tripo `rig`, `animate` and `segment` operate on an exact completed revision; follow [docs/CHARACTERS.md](docs/CHARACTERS.md). Segment uploads its current GLB; rig uploads a copy rotated from actual `rig_forward_axis` (default +Z) to provider +X, preserving geometry/materials and both source/prepared hashes. Restore orientation through a common rig/mesh parent. Animation inherits that orientation and the recorded rig task. Keep stage checkpoints and use `resume-tripo-process` after interruption; an unknown POST outcome must not be retried automatically.
