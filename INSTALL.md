...
| Explicit Kimodo motion | Isolated Linux/WSL CUDA environment, motion model and local text encoder | User-selected learned humanoid motion on an existing rig; preserved clips and separate target-quality review |
...
Run a real reference-image generation using [QUICKSTART.en.md](QUICKSTART.en.md). Inspect numeric results and open the local PNG renders with the agent's image-inspection tool. Deliver the GLB/source and observations. This completes the normal asset workflow without an engine, browser or web server.
...
Local inference uses installed models without API credentials. Readiness checks do not prove successful rigging or segmentation: run the relevant [character workflow](docs/CHARACTERS.en.md) and inspect its actual output. SkinTokens is a rig draft that can be refined with a local script. Presets, custom authoring and clip merging use the normal Blender installation. The agent, not the end user, chooses segmentation prompts from the prepared renders and maps unknown bone names from observed joints.
...
The same Tripo-only installation supports explicitly selected Tripo rigging, preset animation and semantic segmentation of completed revisions, including TRELLIS/import assets. Read [docs/TRIPO.en.md](docs/TRIPO.en.md#rigging-animation-and-segmentation) for paid processing and [docs/CHARACTERS.en.md](docs/CHARACTERS.en.md) for shared review/import rules. This cloud option needs no local postprocessing weights; the default local backends are installed separately above.
...
Kimodo is a learned humanoid-motion profile used only when the user explicitly selects Kimodo, independent of mesh generation. Run `uv run --no-sync python scripts/bootstrap_kimodo.py` only for that selected work; generic animation requests do not select this profile. It installs pinned Python/CUDA dependencies and model snapshots in `.runtime/kimodo`, using Linux or WSL `Ubuntu-24.04` on Windows; `--wsl-distribution` selects another installed distribution. The selected Linux user needs `uv` and `g++`. The local text encoder requires authorized access to Meta-Llama-3-8B-Instruct; put a read token in `<runtime-root>/.secrets/hf_token` or use the HF credential environment. `--skip-models` prepares only the environment. See [Kimodo setup and usage](docs/KIMODO.en.md) for pins, requirements, mapping, separate readiness/inference states and test scope. This profile is not required for static edits, object motion, procedural presets or clip merging.
...
Preserve `.assets/` and local configuration. Do not use `git clean -fdx` or remove `.runtime/` as a general repair step. See [troubleshooting](docs/TROUBLESHOOTING.en.md) for process, download and job failures.
