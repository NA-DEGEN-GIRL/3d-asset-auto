...
1. Convert the request to a validated JSON spec. Use absolute paths for reference images from another project. State any material assumptions once; use existing project conventions when possible. Explicitly asking for Tripo authorizes one standard generation per requested asset without another credit confirmation. `tripo` options are automatic and `max_credits` is optional, defaulting to 100; standard generation is estimated at 30 credits, and a smaller user budget wins. Run `tripo-plan` and read-only `tripo-balance` internally, then continue within that scope without pausing for approval. The guard covers the estimate, not the vendor's final bill or a retry budget. Do not print or embed the API key.
