...
4. Open actual local PNGs. For a new result inspect all five views; for a narrow edit start with affected views. For segmentation inspect the part previews; for animation inspect sampled motion frames and the preset's dense ground report when available. Custom scripts need checks appropriate to their motion. Record specific findings with `review`. Numeric or loader success is not visual approval.
