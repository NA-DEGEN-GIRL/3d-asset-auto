...
- Identify the effect's purpose, target conditions and requested finish. Set observable technical and expressive acceptance criteria against the adopted references and the user's quality target; do not lower them to whatever the starting component already does. Reuse project conventions; do not choose an engine or open a viewer for an unspecified target.
