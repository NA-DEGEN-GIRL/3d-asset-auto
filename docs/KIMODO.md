...
생성 후에는 **리그 적용 → 다각도·재생 검토 → 필요한 Blender 보정 → 최종 GLB 재검토**를 수행합니다. [단계별 진단](QUALITY.md#리그와-동작의-단계별-진단)에 따라 보존된 원본 모션과 대상 골격·메시·장비·출력 중 결함이 생긴 단계를 구분합니다. 모션 의미가 맞고 적용 후에 문제가 생겼다면 기존 추론을 유지하고 `blender-edit`로 뼈 대응·리그·가중치·접촉·타이밍 등 확인된 원인에 맞는 부분을 수정합니다. 새 추론이 필요한지는 원본 모션의 문제와 작업 예산을 근거로 판단합니다.

목표 자세나 동작 의미가 불명확하거나 보정이 반복해 실패하면 [동작 참고 자료](QUALITY.md#동작-참고-자료-활용)로 주요 자세·접촉 사건을 정리하고 실제 결과와 비교합니다. 현재 Kimodo 어댑터의 생성 입력은 텍스트이며 자세 시트·영상 입력이나 자동 모션 추출은 지원하지 않습니다. 참고는 LLM의 프롬프트 구체화와 Blender 보정 판단에 사용합니다.
