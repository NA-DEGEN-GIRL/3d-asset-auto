...
복잡하거나 창의적인 동작은 초기 설계부터 [이미지·영상 참고 자료](QUALITY.md#동작-참고-자료-활용)로 주요 자세·접촉 사건을 정리하고 실제 결과와 비교합니다. 현재 Kimodo 어댑터의 생성 입력은 텍스트이며 자세 시트·영상 입력이나 자동 모션 추출은 지원하지 않습니다. 참고는 LLM의 프롬프트 구체화와 Blender 보정 판단에 사용합니다.
