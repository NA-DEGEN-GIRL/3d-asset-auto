...
이 명령은 기존 원격 작업을 조회하고 결과 다운로드·Blender 처리를 이어갑니다. 새 유료 task를 만들지 않습니다. 이미 완료된 revision은 다시 덮어쓰지 않으며, 수정은 기존 `edit`로 새 revision에 저장합니다. 로컬 수정에는 Tripo 재호출이 필요하지 않습니다.
