...
For complex, creative, unfamiliar or unclear motion, **plan image and temporal references during initial design**. Images clarify key poses and appearance; inspected existing motion/video or authorized video generation can clarify important transitions, rhythm and contact order. Do not wait for repeated failure before considering video. Use an available image-generation tool when pose sheets help, and follow [generated video reference guidance](MOTION_REFERENCES.en.md) when animating a reference image with Grok Imagine or another available tool. Reuse sufficient material and keep reference work within the task budget; simple clear edits do not require new media.

Define [the expression to retain per clip](MOTION_REFERENCES.en.md#turn-references-into-actual-edits), recording source paths/URLs, generated status, adopted/excluded panels or times and associated events. Do not lower targets to poses that are convenient for the current rig. Use staged diagnosis below to distinguish target/result differences, skeletal motion and surface deformation problems.
...
Different panel numbers do not prove different phases. Repeated poses or swapped limbs/supports must not be counted as progression or valid alternation. Repeated settled/held poses can be intentional but must be distinguished from motion. More panels or colored labels alone do not establish consistency. If sequencing/contact/timing cannot be resolved, or contradictions persist after a targeted correction, switch to frames from inspectable existing motion or reviewed video instead of indefinitely expanding the same sheet. Generated video also needs structural, contact and temporal-consistency review, and must be distinguished from recorded footage. Existing Kimodo sources can help, but new Kimodo installation/inference still requires explicit selection. Inspect alternative references too, recording accepted/excluded portions and remaining limits.

Still-image order or spacing does not verify speed, rhythm or transitions. Use [reference-to-final comparison](MOTION_REFERENCES.en.md#compare-reference-and-final-motion) to align events and viewing conditions while preserving time information for direct pose, trajectory and transition comparison. References support the agent's design and Blender decisions; automatic image/video-to-motion extraction is not implemented.

## Complete each requested clip

Track each requested clip's purpose/criteria, adopted expression targets, references, actual review evidence, technical defects, expression gaps and status. A few successful representative clips cannot approve the rest. Keep records concise and useful rather than prescribing a document size.
...
Beyond joint probes, **test adopted target poses and movement paths on the actual rig**. If the result is smaller or differs in direction or lead/follow, distinguish relevant causes such as fixed bend/end-effector directions, joint limits, mapping, axes, constraints or weights. Test control/constraint hypotheses on a copy; the current authoring method's convenient range is not an inevitable model limit. Choose edits from observed target/result differences.
...
For each observed defect, record the failed criterion, location/clip/time, revision and reproducible view/evidence path. For motion, connect **adopted reference target → observed difference → cause hypothesis → repair → new revision/export → comparison under the same conditions**, separating observations from unverified hypotheses. If duration changes, align the same event. Check overall appearance and affected neighbors. A flattering new angle or a successful script is not proof of repair.

Recheck likely regressions in the final GLB. Penetration corrections may damage joint direction/volume or **lose adopted amplitude, trajectories and rhythm**; timing/interpolation changes may break contacts or loop boundaries. Do not default to shrinking motion to avoid penetration or surface collapse; consider cause-specific pose, constraint, rig or weight corrections. If purpose or actual constraints require reduction, record the reason, lost expression and effect on acceptance without hiding unmet targets.

Preserve data outside the edit: mesh, skin, material and existing clips. Identical names do not prove unchanged content/speed. Use hashes/data comparisons, or corresponding data and time-based results with justified tolerances when export representation changes.
...
Numerical, visual, functional and engine-import evidence are separate. In motion review notes, **judge technical stability and adopted expression separately**. Passing penetration, twisting, grounding, loop or export-preservation criteria leaves a clip incomplete if core expression targets remain unmet. Opening references or counting renders does not prove expression was achieved. One kind of evidence cannot erase an observed failure in another. “Prototype” may describe an agreed detail level, not waive required behavior. Do not silently lower criteria to fit the result.

Plan repair effort around clip count/complexity, importance, scope and cost, respecting user constraints. Do not universalize one asset's tolerances, angle limits or iteration count. Finish when relevant criteria are supported. If the same gap persists, reconsider the cause hypothesis and control/authoring method rather than only nudging values within the same restricted approach. If the remaining budget cannot support a justified repair or is exhausted, preserve the best revision and report incomplete work: remaining technical defects/expression gaps, conditions, impact, attempted changes and next needs. Local repair effort does not authorize additional paid generation; stay within the selected service's existing authorization. Apply [Tripo spending rules](TRIPO.en.md) when using Tripo.
