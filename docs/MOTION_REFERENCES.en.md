...
1. **Define each clip's targets.** Identify required poses, event order, contact maintenance/release, travel and playback purpose. Clarify creative motions through meaningful phases without omitting required continuous transitions.
...
4. **Separate accepted and rejected observations.** Inspect progression and neighboring event frames with timestamps. Look for side swaps, disappearance/duplication, shape drift, occluded contacts, cropped equipment and excessive holds. Check actual periods and multiple-cycle boundaries for loops. Smooth video alone cannot approve physics, grips or looping.
5. **Apply in Blender and inspect the final GLB.** Record adopted poses/times/rhythm and adapt them to target proportions, joint range and gameplay speed. Transferring only timing is valid, but is not pose extraction or new learned motion. Keep skeleton, equipment, contact constraints, corrective morphs and usage-event times coordinated.
