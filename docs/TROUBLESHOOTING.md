...
`inspect ASSET_ID REVISION`으로 실제 오브젝트 이름을 읽습니다. AI 생성물이 `Mesh_0` 하나라면 의미별 손잡이·뚜껑이 분리된 상태가 아닙니다. 이름을 추측해서 반복 요청하지 않습니다. 필요한 경우 요청한 Tripo `segment`를 실행하고 `part-previews.json`의 개별 PNG로 확인한 부품에만 `rename`을 적용합니다. 자동 라벨이나 떨어진 기하 자체를 의미 부품의 증거로 삼지 않습니다.
...
부모 `revision`이 완료되었는지와 요청이 정확한 모델을 가리키는지 확인합니다. 리깅·분리는 정적 부모를 사용하며, `animate`는 우리 런타임의 완료된 Tripo rig/animate revision과 `rig_task_id`가 필요합니다. 외부 리그를 import한 것만으로 이 원격 ID가 생기지는 않습니다.

리깅 가능 검사에서 거절되면 먼저 실제 렌더의 정면과 `rig_forward_axis`가 일치하는지 확인한 뒤 형상·포즈와 거절 사유를 살핍니다. 기본 +Z 정면은 런타임 기준이며 외부 파일의 방향을 보장하지 않습니다. 어댑터가 +X 입력 변환과 출력 복원을 맡으므로 각도를 임의로 반복 추측하지 않습니다. 원격 후처리 중단은 `resume-tripo-process ASSET_ID REVISION --async`로 **미완료 자식**을 이어갑니다. 새 작업을 중복 제출하지 않습니다. 단계별 기록과 불명확한 제출 결과의 처리 방법은 [후처리 복구](CHARACTERS.md#기록과-복구)를 따릅니다.
...
걷기에서 발이 바닥 아래로 내려가면 검사·프리뷰의 샘플 바닥 경고와 원격 원본을 비교합니다. 실제 좀비 테스트에서는 원격 GLB에도 같은 편차가 있어 시각 검토를 실패로 기록했습니다. 자동 높이 이동으로 결함을 숨기거나 API 성공만으로 보행 품질을 승인하지 않습니다.
