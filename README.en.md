...
| Evaluate the VFX skill in a fresh LLM session | [Independent-session testing](docs/VFX_TESTING.en.md): real requests, per-effect results and failure evidence |
...
Game VFX uses a separate [game-vfx skill](.agents/skills/game-vfx/SKILL.md) to **inspect the actual project's renderer, version, device, camera, interactions and quality target before choosing authoring and playback methods**. Blender is a preferred available local tool for relevant ingredient authoring and baking; procedural fields, curves, particles and shaders are also valid methods. Simulation is an option for achieving the requested expression, not a quality guarantee. The initial implementation provides a spatial fire shader, an existing ice-fracture clip, branching lightning and a [Three.js gallery](docs/GAME_VFX_DESIGN.en.md). Use `3d-assets` when a separate physical model such as an ice chunk or meteor, or mesh animation, is needed; effects without model requirements need no TRELLIS.

The fire example is not fluid simulation, and the ice uses precomputed movement on a flat surface. Separate reusable sources, materials, visual intent and event contracts from engine-specific shaders, particles and scenes; the latter do not all fit in a GLB. This is not a general VFX generation CLI/MCP, arbitrary-terrain collision system, automatic cross-engine converter or commercial-quality guarantee. Existing [VFX experiments](docs/VFX.en.md) remain available, with references, multiview/playback review and repairs performed per effect. With no engine selected, prioritize the requested output format and include suitable ingredients and specifications; launch a web preview only when requested.
