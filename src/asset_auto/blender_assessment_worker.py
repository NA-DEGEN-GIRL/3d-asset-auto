...
from motion_quality import analyze, important_frames, sample_times
...
def run(request):
    ...
    if request["render"]:
        depth = 0
        while len(chosen) < request["max_render_frames"]:
            candidates = [(name, report["frames"][depth]) for name, report in reports.items() if len(report["frames"]) > depth]
            if not candidates:
                break
            chosen.extend(candidates[:request["max_render_frames"] - len(chosen)])
            depth += 1
    if chosen:
        character.select_preview_clip(None)
        character.frame_at(0)
        ...
        for index, (name, frame) in enumerate(chosen):
            character.select_preview_clip(name)
            character.frame_at(frame["time_seconds"])
            frame["file"] = f"moment-{index:03d}.png"
            bpy.context.scene.render.filepath = str(out / frame["file"])
            bpy.ops.render.render(write_still=True)
    for name, report in reports.items():
        report["rendered_frames"] = sum("file" in frame for frame in report["frames"])
