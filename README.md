...
선택한 인간형 작업은 [참고 검토 → 리그 확인 → Kimodo 초안 → Blender 보정 → 최종 비교](docs/KIMODO.md#선택한-kimodo-작업의-제작-흐름)를 우선 검토합니다. Kimodo는 3D 동작의 출발점을 제공하며, 최종 클립은 기술적 안정성과 채택한 표현 목표를 모두 확인해 완성합니다.
...
어렵거나 창의적인 동작은 초기 설계부터 이미지와 시간 자료를 함께 계획합니다. 사용 가능한 Grok Imagine 등으로 기준 이미지의 영상을 만들고 선택한 자세·타이밍을 Blender에서 참고할 수 있습니다. 생성 영상의 구조·접촉도 먼저 검토하며, 자동 영상→3D 모션 추출은 제공하지 않습니다. Grok·FFmpeg는 이 선택적 참고 작업을 위한 도구이며 기본 설치에 추가되지 않습니다.
