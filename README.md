...
**리깅·애니메이션·부품 분리도 기본은 로컬입니다.** SkinTokens로 골격·가중치를 추론하고, Blender로 기본 동작을 만들며, GeoSAM2와 LLM의 이미지 검토로 부품을 나눕니다. 해당 작업에 필요한 모델만 설치하고 API 키 없이 사용합니다. Tripo는 이 작업들에서도 명시적으로 고르는 대안입니다.
...
| 기본 로컬 동작 | 실제 뼈에 Blender IK로 `idle`·`walk`·`run`, 동작별 새 revision |
...
**현재 제한:** 동작은 이족 기본 프리셋이며 모션 캡처·일반 생물 동작이 아닙니다. 사용자 모션, 다중 클립 GLB 병합, 임의의 리그·가중치 편집, TRELLIS 멀티뷰, 애니메이션용 리토폴로지와 텍스처 재베이크는 제공하지 않습니다. 부품 이름·경계·재질은 실제 렌더로 확인하며 분류하지 못한 면은 보존합니다. 멀티뷰 생성은 Tripo 옵션에서만 지원합니다.
...
리깅·분리가 필요한 작업에서는 [로컬 후처리 모델](INSTALL.md#local-postprocessing-on-demand)을 추가합니다. 모든 소품 작업에 이 모델들을 받거나 후처리를 실행할 필요는 없습니다. 로컬 기본 동작 생성은 기존 Blender와 리그를 사용합니다.
...
2026-09-13 기준 v0.1 구현에서 확인한 결과:
