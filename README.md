...
| 게임 스킬 효과·충격파·파티클 제작 | [VFX 가이드](docs/VFX.md) — 표현 선택·수명·검토와 Three.js 웹 테스트 |
...
| 게임 VFX 작성 | 이미지·영상 참고를 사용하는 제작·검토 지침, Three.js 충격파·생성 영상 플립북 예제; 범용 자동 생성·엔진 간 변환은 미구현 |
...
게임 스킬 효과는 [VFX 경로](docs/VFX.md)에서 메시 동작·텍스처·파티클·셰이더 중 필요한 표현을 선택합니다. 효과용 점·링·리본은 직접 작성할 수 있고, 별도의 실제 소품 모델은 기존 생성 원칙을 따릅니다. 임의 셰이더·방출기는 GLB 하나로 전달되지 않으므로 네이티브 소스와 재생 명세를 포함합니다. 웹 미리보기는 요청한 경우에만 만듭니다.
