...
| 게임의 3D 스킬 효과·파편·파티클 제작 | [VFX 가이드](docs/VFX.md) — 기능 범위, 참고 → 낙하·파괴 저작, 검토와 선택적 웹 테스트 |
...
| 게임 VFX 작성 | 이미지·영상 참고 → Blender/대상 렌더러 저작 지침. 파편의 강체 계산 → 공유 클립 → GLB 소규모 검사 통과; 전용 자동 파쇄·게임 충돌 연동은 미구현 |
...
게임 스킬 효과는 [VFX 경로](docs/VFX.md)에서 기존 기능·별도 저작·부족한 기능을 먼저 구분합니다. 예를 들어 얼음 낙하·파괴는 참고 자료 → 실제 덩어리 모델 → Blender 파편·운동 저작 → GLB 또는 대상 엔진의 사건 처리로 제작할 수 있습니다. **이미지·영상 참고를 평면 재생하는 것으로 3D 효과를 대신하지 않습니다.** 효과용 점·링·리본은 직접 작성할 수 있고, 새 덩어리·소품 모델은 기존 생성 원칙을 따릅니다. 물리 계산·임의 셰이더·방출기·게임 로직은 GLB만으로 전달되지 않으므로 필요한 원본과 재생 명세를 포함합니다. 기존 영혼 불꽃은 2D 플립북 실험이며, 웹 미리보기는 요청한 경우에만 만듭니다.
