...
| 게임 효과 제작·검토 | [game-vfx 스킬](.agents/skills/game-vfx/SKILL.md), [설계·실행 안내](docs/GAME_VFX_DESIGN.md), [VFX 제작과 실험](docs/VFX.md) — 대상 프로젝트에 맞는 제작·재생 선택 |
| 새 LLM 세션에서 VFX 스킬 평가 | [독립 세션 테스트](docs/VFX_TESTING.md) — 실제 요청, 효과별 결과와 실패 근거 |
...
게임 VFX는 별도 [game-vfx 스킬](.agents/skills/game-vfx/SKILL.md)에서 **실제 프로젝트의 렌더러·버전·장치·카메라·상호작용·품질 목표를 확인한 뒤 제작과 재생 방식을 선택**합니다. Blender는 필요한 재료 저작·베이크에 우선 활용할 수 있는 로컬 도구이며, 절차적 필드·곡선·입자·셰이더도 정상 제작 경로입니다. 시뮬레이션은 필요한 표현을 얻는 선택지이며 품질 보증이 아닙니다. 첫 구현은 공간 화염 셰이더, 기존 얼음 파쇄 클립, 분기 번개와 [Three.js 갤러리](docs/GAME_VFX_DESIGN.md)를 제공합니다. `3d-assets`는 별도의 얼음 덩어리·운석 같은 모델이나 메시 애니메이션이 필요할 때 사용하며, 모델이 필요 없는 효과에는 TRELLIS가 필요하지 않습니다.

불 예제는 유체 시뮬레이션이 아니며 얼음은 미리 계산한 평평한 바닥의 운동입니다. 재사용할 원본·재료·표현 의도·사건 명세와 엔진별 셰이더·입자·장면 구현을 구분하며, 후자가 GLB에 모두 들어가지는 않습니다. 범용 VFX 생성 CLI/MCP, 임의 지형 충돌, 다른 엔진 자동 변환이나 상용 게임 품질을 보장하는 기능은 아닙니다. 기존 [VFX 실험](docs/VFX.md)도 보존하며 효과별 참고·다각도·재생 검토와 수정을 이어갑니다. 엔진이 미정이어도 요청한 출력 형식을 우선하고 필요한 재료와 명세를 함께 전달하며, 웹 미리보기는 요청한 경우에만 실행합니다.
